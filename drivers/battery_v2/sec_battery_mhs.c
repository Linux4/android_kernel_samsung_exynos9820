...
int sec_bat_set_charging_current(struct sec_battery_info *battery)
{
	...
	if (is_nocharge_type(battery->cable_type)) {
		...
		if ((battery->store_mode || !battery->charging_enabled) && is_hv_wire_type(battery->select->cable_type) && (battery->capacity >= 5)) {
...
skip_current_monitor:
	...
	if (!is_nocharge_type(battery->cable_type) && (battery->cable_type != SEC_BATTERY_CABLE_OTG) && !battery->charging_enabled) {
		int chg_mode;

		dev_info(battery->dev,
			 "%s: @battery->capacity = (%d), battery->status= (%d), battery->charging_enabled=(%d)\n",
